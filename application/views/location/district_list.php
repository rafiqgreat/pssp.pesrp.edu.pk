          ...
          <div class="card-header d-flex p-0">
            ...
            <div class="ml-auto p-2">
              <?php if (hasPermissions('district_add')): ?>
                <a href="<?php echo url('location/district_add') ?>" class="btn btn-primary btn-sm"><span class="pr-1"><i class="fa fa-plus"></i></span> <?php echo lang('new_district') ?></a>
              <?php endif ?>
            </div>
          </div>
                  ...
                  <tr>
                    ...
                    <td>
                      <?php if (hasPermissions('district_edit')): ?>
                        <a href="<?php echo url('location/district_edit/' . $row->district_id) ?>" class="btn btn-sm btn-default" title="<?php echo lang('district_edit') ?>" data-toggle="tooltip"><i class="fas fa-edit"></i></a>
                      <?php endif ?>
                      <?php if (hasPermissions('district_delete')): ?>
                        <a href="<?php echo url('location/district_delete/' . $row->district_id . '?table=districts&key=district_id') ?>" class="btn btn-sm btn-default" onclick='return confirm("Do you really want to delete this district ? \nIt may cause errors where it is currently being used !!")' title="<?php echo lang('district_delete') ?>" data-toggle="tooltip"><i class="fa fa-trash"></i></a>
                      <?php endif ?>
                    </td>
                  </tr>
                <?php endforeach ?>
